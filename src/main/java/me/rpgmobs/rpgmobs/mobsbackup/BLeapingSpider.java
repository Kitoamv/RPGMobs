...
public class BLeapingSpider implements Listener {
    ...
    public static void createBLeapingSpider(Location location){
        ...
        }.runTaskTimer(plugin, 01, 20L);
    }

    @EventHandler
    public void onDamage(EntityDamageByEntityEvent event){
        if(event.getDamager() instanceof Spider){
            if(event.getDamager().getCustomName() != null && event.getDamager().getCustomName().equals(ChatColor.DARK_RED + "Aranha Seguidora")){
